// Brand v3 — primary visual identity for «Умный ученик». Uses a verified cache-safe logo asset.
(() => {
  const LOGO = '/smart-student-logo-v3.png';
  let scheduled = false;
  ...
  function decorateLockup(lockup) {
    ...
    } else if (!img.src.includes('smart-student-logo-v3.png')) {
      img.src = LOGO;
    }
  }
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', scheduleBrand, { once: true });
  else scheduleBrand();
